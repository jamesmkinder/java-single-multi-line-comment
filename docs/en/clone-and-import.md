...
`git clone https://github.com/ByteLegendQuest/java-single-multi-line-comment`

A project directory named `java-single-multi-line-comment` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-single-multi-line-comment` directory which is created just now.
